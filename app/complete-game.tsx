      ...
      {view === "progression" && <div className="progression-page page-enter">
        ...
        <div className="progression-grid">
          ...
          <aside className="advancement-panel complete-card">
            ...
            {nextSequence ? <>
              <div className="advancement-meter"><div><span>魔药消化</span><strong>{game.digestion}%</strong></div><span><svg className="progress-fill" viewBox="0 0 100 1" focusable="false" aria-hidden="true"><rect width={Math.max(0, Math.min(100, game.digestion))} height="1" /></svg></span><p>{currentSequence.acting}</p></div>
              ...
              <div className="advancement-meter"><div><span>配方知识</span><strong>{game.formulaKnowledge}%</strong></div><span><svg className="progress-fill" viewBox="0 0 100 1" focusable="false" aria-hidden="true"><rect width={Math.max(0, Math.min(100, game.formulaKnowledge))} height="1" /></svg></span><p>{game.formulaKnowledge >= 100 ? "完整配方已核验，材料清单可以用于行动契约。" : "需要研究、交易或从其他势力获取更多配方知识。"}</p></div>
               <div className="advancement-meter"><div><span>仪式准备</span><strong>{game.ritualReadiness}%</strong></div><span><svg className="progress-fill" viewBox="0 0 100 1" focusable="false" aria-hidden="true"><rect width={Math.max(0, Math.min(100, game.ritualReadiness))} height="1" /></svg></span><p>{ADVANCEMENT_RITUALS[game.pathwayId]?.[nextSequence.rank] ?? "仪式条件尚未由知识库证据确认。必须先获取并验证，系统不会自动编造。"}</p><button className="ritual-plan" onClick={() => applySuggestion(`为晋升序列${nextSequence.rank}·${nextSequence.name}核验并准备仪式：${ADVANCEMENT_RITUALS[game.pathwayId]?.[nextSequence.rank] ?? "先从可靠来源取得仪式条件并完成知识库核验"}。先拆分条件、验证安全措施，不在条件不足时服食魔药。`, "cherwood")}>把仪式写入自由行动</button></div>
