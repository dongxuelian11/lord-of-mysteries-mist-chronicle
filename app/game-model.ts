...
export const ORGANIZATION_KINDS: OrganizationKind[] = [
  ...
  { id: "charity", name: "慈善互助会", defaultName: "灰烬互助会", cover: "施粥、义诊与收容登记", description: "以救济与义诊进入平民社区。", boon: "基层信任与病例网络", debt: "病人和组织安全经常冲突" },
  { id: "archive", name: "私人档案馆", defaultName: "钟楼档案馆", cover: "文献代查与契约保管", description: "以档案与文书业务隐藏知识追索。", boon: "档案、仪式语言与学者关系", debt: "知识更容易进入教会视野" },
  { id: "trading", name: "贸易商行", defaultName: "雾港商行", cover: "进出口代理与仓储", description: "以货单与码头渠道构成经济外壳。", boon: "材料、资金和码头关系", debt: "旧合同与灰色客户不会消失" },
  { id: "sect", name: "秘密结社", defaultName: "夜语读书会", cover: "晚间读书会与旧友聚会", description: "以私密聚会为壳，吸纳同类。", boon: "守密文化与自愿追随", debt: "一旦暴露即被视为非法组织" },
];

export function organizationKindById(id: string) {
  return ORGANIZATION_KINDS.find((item) => item.id === id) ?? ORGANIZATION_KINDS[0];
}

export function createOpeningMission(origin: Pick<PlayerOrigin, "organizationKind" | "identityLabel">): PressureMission {
  const templates: Record<string, { id: string; title: string; premise: (identity: string) => string; consequence: string; hints: string[]; urgency: number; deadline: number }> = {
    detective: {
      id: "opening-detective", title: "失踪委托人的半份名单", urgency: 72, deadline: 3,
      premise: (identity) => `作为${identity}，你收到一份没有署名的委托：一名常客留下半份名单后消失。名单只写着地址与时间，最后一栏被雨水浸透。`,
      consequence: "若置之不理，委托人可能已经遇险，这份名单也会流进他人手里。",
      hints: ["核对名单上的地址是否真实", "查找委托人最后出现的地点", "询问是否有人见过他离开", "把名单交给教会或警方备案"],
    },
    charity: {
      id: "opening-charity", title: "诊所里没有名字的病人", urgency: 70, deadline: 3,
      premise: (identity) => `作为${identity}，诊所昨晚收治一名没有名字的病人。他手上有黑斑，病历一栏被人撕掉，今早他独自离开，留下一个旧布袋。`,
      consequence: "黑斑可能意味着污染；布袋里的东西可能牵连更多病人。",
      hints: ["检查布袋里的物品", "对照昨夜值班记录", "询问附近住户是否见过他", "把情况报给可信的教会人员"],
    },
    archive: {
      id: "opening-archive", title: "被封存的目录缺了一页", urgency: 68, deadline: 3,
      premise: (identity) => `作为${identity}，你保管的封存目录被借阅者翻动过：一页关于旧仪式的条目被撕走，借阅登记却没有对应签名。`,
      consequence: "被撕走的条目可能让外人顺藤摸瓜找到更多封存资料。",
      hints: ["核对借阅登记的笔迹", "检查封存室的出入记录", "确认最近接触过目录的人", "决定是否报告教会"],
    },
    trading: {
      id: "opening-trading", title: "货仓里多出的密封箱", urgency: 71, deadline: 3,
      premise: (identity) => `作为${identity}，清点货仓时发现一只没有货单的密封箱，封蜡上有陌生徽记，码头管事说不清它何时入库。`,
      consequence: "箱内物品一旦被海关或他人发现，商行的合法掩护会立刻失效。",
      hints: ["查明箱子入库的时间", "核对当周卸货记录", "询问码头工人是否见过搬运者", "决定开箱、封存还是转交"],
    },
    sect: {
      id: "opening-sect", title: "旧成员留下的密信", urgency: 69, deadline: 3,
      premise: (identity) => `作为${identity}，一位旧成员在聚会散场后留下密信，信里说“有人在按名单找我们”，随后没有再出现。`,
      consequence: "若名单真的存在，结社的隐蔽身份可能已经暴露。",
      hints: ["核对旧成员最近接触过的人", "检查信纸来源与墨水", "确认是否真有名单流出", "决定转移据点还是主动接触对方"],
    },
  };
  const template = templates[origin.organizationKind] ?? templates.detective;
  return {
    id: template.id,
    title: template.title,
    premise: template.premise(origin.identityLabel),
...
export function createInitialGame(pathwayId: PathwayId = "seer", origin?: Pick<PlayerOrigin, "organizationKind" | "identityLabel" | "organizationName">): GameState {
  ...
  return {
    ...
    playerOrigin: {
      ...
      gender: "",
      age: "",
      organizationName: origin?.organizationName || organizationKindById(openingOrigin.organizationKind).defaultName,
      organizationKind: openingOrigin.organizationKind,
      organizationKindLabel: organizationKindById(openingOrigin.organizationKind).name,
      organizationCharter: "保护组织成员与无辜者；证据不足时不公开指控；未知高位威胁下优先撤退。",
    },
    nameExposure: 4,
    knownAliases: [],
    week: 1,
    ...
    departmentReports: [],
    organizationIssues: [],
    inventory: [...opening.inventory, { id: "ritual-kit", name: "基础仪式器具箱", category: "仪式器具", quantity: 1, location: "简易仪式室", keeper: "罗文·布莱克", risk: "低" }, { id: "cover-papers", name: "组织合法身份文件", category: "身份文件", quantity: 1, location: "密议室暗柜", keeper: "塞德里克·霍尔", risk: "一旦丢失，主据点掩护将立刻失效。" }],
    missions: [createOpeningMission(openingOrigin)],
    ...
    evidenceNodes: opening.evidenceNodes.map((item) => ({ ...item, tags: [...item.tags] })),
    evidenceLinks: opening.evidenceLinks.map((item) => ({ ...item })),
    opportunities: opening.opportunities.map((item) => ({ ...item, requirements: [...item.requirements] })),
