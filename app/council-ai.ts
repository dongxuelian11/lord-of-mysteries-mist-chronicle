...
export async function generateCouncilReplies(config: AiConfig, game: GameState, topicText: string, topicId?: string): Promise<CouncilTopicMessage[]> {
  ...
  const speakerLore = Object.fromEntries(await Promise.all(members.map(async (member) => {
    ...
    const lore = await retrieveLoreContextAsync(LORE_RECORDS, { query: `${topicText} ${specialty}`, audience: { kind: "actor-private", principalRef: `actor:${member.id}`, purpose: "actor-council", knownLoreIds, topicGrants }, limit: 8, maxChars: 3200, week: game.week, gameDate: game.date, horizon });
  ...
  const result = (await Promise.all(members.map(async (member, memberIndex) => {
    const worldProjection = game.worldKernel
      ? projectWorldForAudience(game.worldKernel, { kind: "actor", holderId: member.id })
      : null;
    const authorizedKnowledge = worldProjection?.knowledge.slice(-12) ?? [];
    const payload = {
      ...sharedPayload,
      ...
      scheduled: game.schedule
        ...
        .map((item) => ({ title: item.title, rawIntent: item.rawIntent, risk: item.risk })),
      currentWorld: worldProjection ? {
        week: worldProjection.currentWeek,
        date: worldProjection.currentDate,
        events: worldProjection.events.slice(-8),
        observations: worldProjection.observations.slice(-10),
        knowledge: worldProjection.knowledge.slice(-12),
      } : null,
      lastWeek: game.chronicle[0] ? {
        results: game.chronicle[0].results
          .filter((item) => item.contract.leaderId === member.id || item.contract.memberIds.includes(member.id))
          .map((item) => ({ title: item.title, outcome: item.outcome, findings: item.findings })),
      } : null,
    ...
    const raw = extractJson(await callModel(
      ...
      { task: "council-reply", json: true, maxTokens: 1400, temperature: .96 },
    ));
