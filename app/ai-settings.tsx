...
import { AiConfig, AiProviderId, AiQuality, DEEPSEEK_FLASH_PRESET, isLoopbackInferenceEndpoint } from "./ai-client";
...
export default function AiSettings({ config, rememberKey, secureStorageAvailable, credentialConfigured, connection, turnStages, showDiagnostics, autoDecision, onAutoDecision, draftPathway, onChange, onRememberKey, onTest, onSave, onClearKey, onPathway, onNewGame }: Props) {
  const provider = config.provider ?? "deepseek";
  const desktopInference = typeof window !== "undefined" && Boolean(window.mistInference && window.mistCredentials);
  const ready = Boolean(config.endpoint.trim() && config.model.trim() && (
    desktopInference
      ? provider === "compatible"
        ? isLoopbackInferenceEndpoint(config.endpoint)
        : credentialConfigured || config.apiKey.trim()
      : config.apiKey.trim()
  ));
  function selectProvider(value: AiProviderId) {
    if (value === "deepseek") onChange({ ...DEEPSEEK_FLASH_PRESET, apiKey: config.apiKey });
    else onChange({ provider: "compatible", endpoint: desktopInference ? "http://127.0.0.1:11434/v1" : "", model: "", quality: config.quality ?? "balanced", apiKey: "" });
  }
  return <>
    ...
    <section className="provider-choice" aria-label="模型服务商">
      ...
      <button className={provider === "compatible" ? "selected" : ""} onClick={() => selectProvider("compatible")}><Network size={17} /><span><strong>本机兼容接口</strong><small>仅限本机回环端点与自定义模型名</small></span>{provider === "compatible" && <Check size={15} />}</button>
    </section>

    {provider === "deepseek" && <div className="deepseek-note"><Zap size={15} /><p><strong>已使用官方 V4 Flash 预设</strong><span>桌面版只连接官方 api.deepseek.com；请求与密钥由本机主进程处理。</span></p></div>}

    <label className="api-key-field"><span><KeyRound size={13} />API Key</span><input type="password" autoComplete="off" value={config.apiKey} onChange={(event) => onChange({ apiKey: event.target.value })} placeholder={credentialConfigured && desktopInference ? "已由本机主进程保管；留空可继续使用" : provider === "deepseek" ? "sk-… 仅用于调用 DeepSeek" : "本机服务不需要密钥时可留空"} /></label>
    ...
    {(rememberKey || credentialConfigured) && <button className="clear-key-button" onClick={onClearKey}>清除本机持有的 Key</button>}
    ...
    <details className="api-advanced">
      ...
      <div>
        <label><span>接口基础地址</span><input value={config.endpoint} onChange={(event) => onChange({ endpoint: event.target.value })} placeholder="http://127.0.0.1:11434/v1" readOnly={provider === "deepseek"} />{desktopInference && provider === "compatible" && !isLoopbackInferenceEndpoint(config.endpoint) && <small>桌面版只允许 localhost、127.0.0.1 或 ::1。</small>}</label>
        ...
        <label><span>专用世界回应模型（可选）</span><input value={config.worldModel ?? ""} onChange={(event) => onChange({ worldModel: event.target.value })} placeholder="留空则与人物对话共用当前模型" /><small>每周会先生成世界变化，再处理人物对话与小说化叙事。</small></label>
        ...
        <label className="world-bible-field"><span>本局世界补充资料（可选）</span><textarea value={config.worldBible ?? ""} onChange={(event) => onChange({ worldBible: event.target.value.slice(0, 24000) })} placeholder="只补充本局原创组织、历史分支或你想覆盖的裁定。内建设定库无需重复粘贴。" rows={6} /><small>{(config.worldBible ?? "").length}/24000 · 这份补充只用于本局后续世界变化，不会直接出现在人物对话或玩家现状里</small></label>
    ...
    <small className="settings-note">存档与规则判定始终留在本地。桌面版的 DeepSeek 预设只访问官方 api.deepseek.com；兼容接口只允许本机回环地址，密钥不会返回页面代码。</small>
