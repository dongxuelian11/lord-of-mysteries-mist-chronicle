...
export function actingPrinciplesFor(game: Pick<GameState, "pathwayId" | "currentSequence">) {
  return ACTING_PRINCIPLES[game.pathwayId][Math.max(5, game.currentSequence)] ?? [PATHWAYS[game.pathwayId].sequences.find((item) => item.rank === game.currentSequence)?.acting ?? "保持自我锚点"];
}
