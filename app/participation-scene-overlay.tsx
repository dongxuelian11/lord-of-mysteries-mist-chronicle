...
export default function ParticipationSceneOverlay({ scene, loading, error, onDecision, onResume }: {
  ...
  return <div className="participation-lock" role="dialog" aria-modal="true" aria-labelledby="participation-title">
    <section className="participation-scene">
      ...
      <div className="participation-transcript">{scene.turns.length ? scene.turns.map((turn) => <article key={turn.id}><small>你的命令</small><p>{turn.playerIntent}</p><small>现场回应</small><p>{turn.narrative}</p></article>) : <p className="participation-opening">局面已经由此前的决定与现场条件推动，但你还不知道结局。现在可以用任何自然语言亲自处理现场。</p>}</div>
      {error && <p className="participation-error">{error}</p>}
      {complete ? <footer><p>现场过程已经结束。现在查看结局，并让城市继续向前。</p><button onClick={onResume}>查看结果</button></footer> : <footer><label><span>{scene.prompt}</span><textarea value={intent} disabled={loading} onChange={(event) => setIntent(event.target.value)} placeholder="自由输入任何行动、能力使用、战术或撤离命令……" maxLength={1600} /></label><button disabled={loading || !intent.trim()} onClick={() => { const next = intent.trim(); setIntent(""); onDecision(next); }}><Send size={15} />{loading ? "现场正在回应……" : "执行"}</button></footer>}
    </section>
  </div>;
}
