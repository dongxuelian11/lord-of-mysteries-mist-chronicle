    ...
    {props.open && <div className="complete-sheet-backdrop ability-backdrop" onMouseDown={props.onClose}>
      <section className="ability-console" role="dialog" aria-modal="true" aria-labelledby="ability-console-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="ability-console-grid">
          <aside>{props.abilities.map((ability) => <button key={ability.id} className={selected?.id === ability.id ? "selected" : ""} onClick={() => props.onSelect(ability.id)}><span><strong>{ability.name}</strong><small>{ability.verb}</small></span><b>{ability.passive ? "集中1" : `${ability.cost}灵性`}</b></button>)}</aside>
          <article>{selected && <>
            <header><div style={{ "--ability-color": PATHWAYS[props.game.pathwayId].color } as React.CSSProperties}><Eye size={18} /></div><span><small>序列{props.game.currentSequence}可用</small><h3>{selected.name}</h3></span></header>
            ...
            <label><span>你要如何使用它？</span><textarea value={props.intent} onChange={(event) => props.onIntent(event.target.value)} placeholder={`自由描述对象、观察重点和停止条件。例如：我对${props.context.label}集中使用${selected.name}，只确认情绪变化及其触发点；不进行强制干涉。`} maxLength={700} /></label>
            <div className="ability-risk"><CircleAlert size={14} /><span><strong>{selected.passive ? "主动深化被动感知" : selected.risk}</strong><small>{props.game.spirituality < (selected.passive ? 1 : selected.cost) ? "灵性不足：继续将产生透支、污染与失控风险。" : "普通能力点击后直接结算；只有明确高危使用才需要额外确认。"}</small></span></div>
