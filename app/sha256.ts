/** Small synchronous SHA-256 implementation for browser and Node runtimes. */
export function sha256Hex(text: string): string {
  ...
  for (let index = 0; index < 4; index += 1) {
    padded[paddedLength - 8 + index] = (highLength >>> ((3 - index) * 8)) & 0xff;
    padded[paddedLength - 4 + index] = (lowLength >>> ((3 - index) * 8)) & 0xff;
  }
  const constants = [
