...
test("implements the complete simulation systems and accessible Apple-style UI", async () => {
  ...
  const weekResolution = await readFile(new URL("../app/game-engine/week-resolution.ts", import.meta.url), "utf8");
  const worldTurn = await readFile(new URL("../app/game-engine/world-turn-orchestrator.ts", import.meta.url), "utf8");
  const engineRules = `${engine}\n${weekResolution}\n${worldTurn}`;
  const councilFocus = await readFile(new URL("../app/council-focus.ts", import.meta.url), "utf8");
  ...
  assert.match(council, /阅读完整纪事/);
  assert.match(app, /每周纪事都会永久保存在这里/);
  ...
  assert.doesNotMatch(app, /legacy\.version ===|\[5, 6, 7\]|\[10, 11, 12/);
  ...
  assert.match(saveSystem, /nameExposure/);
  ...
  assert.doesNotMatch(council, /周一|周二|周三|周四|周五|周六|周日|action\.days|action\.budget/);
  assert.match(councilFocus, /负责人按你的边界停下了，没有擅自执行/);
  assert.match(council, /目标保留|部分完成|已经停下/);
  assert.doesNotMatch(`${council}\n${councilFocus}`, /\b(?:startDay|timeWindow|executionPlan)\b|action\.(?:days|budget)|第\s*\d+\s*日|改期/);
  ...
  assert.match(engineRules, /actionEvidenceIds/);
  assert.match(worldEnvelope, /行动报告缺少可核验的现场事实/);
  assert.match(engineRules, /refreshOpportunities/);
  assert.match(engineRules, /commitWorldTurn/);
  assert.doesNotMatch(engine, /applyWorldTurn/);
  assert.match(turnCommit, /applyWorldTurn/);
  assert.match(engineRules, /timelineAfterWeek/);
  assert.match(engineRules, /organizationConditions/);
  assert.match(engineRules, /abilitiesFor/);
  ...
  assert.match(engineRules, /playerIssuedNoOrders/);
  assert.match(engineRules, /publicSignals/);
  assert.match(engineRules, /worldSnapshots/);
  assert.match(engineRules, /adjudicatorWorld/);
  assert.match(engineRules, /autonomousAgentProposals/);
  assert.match(engineRules, /planAutonomousAgentsForWeek/);
  assert.match(engineRules, /WORLD_KERNEL_PROTOCOL/);
  assert.match(engineRules, /boundedPayload = finalized\.manifest/);
  assert.match(engineRules, /runtimeAutonomousProposals: autonomousAgentProposals/);
  assert.match(engineRules, /boundedPayload\.runtimeAutonomousProposals/);
  assert.match(engineRules, /key !== "runtimeAutonomousProposals"/);
  assert.match(engineRules, /WORLD_INFERENCE_FROZEN_SCOPE_MISMATCH/);
  ...
  assert.match(engineRules, /kernelDelta/);
  assert.match(engineRules, /authorizedLore/);
  assert.match(engineRules, /actionReports/);
  assert.match(engineRules, /现场述职/);
  assert.match(engineRules, /emergentLead/);
  assert.match(engineRules, /ai-emergent/);
  assert.match(engine, /connectEvidence/);
  assert.match(engine, /transformOrganization/);
  assert.match(engineRules, /buildPivots/);
  assert.match(engineRules, /createFinaleCampaign/);
  ...
  assert.match(council, /三件大事/);
  assert.match(council, /追问推荐负责人/);
  assert.match(council, /干部提出的可编辑方案/);
  assert.match(council, /写入可编辑指令/);
  assert.match(council, /onUseSuggestion\(proposal\.text/);
  assert.doesNotMatch(council, /onUseSuggestion\(proposal\.text[^\n]*scheduleContract|proposal\.text[^\n]*resolveWeek/);
  assert.match(council, /日常事务继续自行运行/);
  assert.match(council, /闭会，等待世界回应/);
  assert.match(council, /BacklundControlMap/);
  assert.match(council, /council-advisers/);
  assert.match(council, /四名负责人/);
  assert.match(council, /每个人只从自己掌握的事实和立场发言/);
  assert.doesNotMatch(council, /member-agendas/);
  assert.match(council, /控制只会因真实行动与各方回应而改变/);
  ...
  assert.match(engineRules, /changes: publicSignals/);
  assert.match(council, /干部讨论/);
  assert.match(council, /整理事实与分歧/);
  assert.match(council, /上一轮回响/);
  assert.match(council, /本轮可知但尚未升级为大事/);
  assert.doesNotMatch(council, /独立世界推演|上周述职|治理议题|形成决议/);
  ...
  assert.match(council, /buildCouncilMatters/);
  assert.match(council, /日常事务继续自行运行/);
  assert.doesNotMatch(council, /foundationSteps|组织奠基/);
