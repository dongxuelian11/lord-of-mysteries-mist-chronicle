...
test("NPC speech is AI generated and a quiet week uses independent planning plus a fixed newspaper", async () => {
  const [game, engine, worldTurn, agentPlanning, council, adjudicatorPrompt, authority, outputAdapter] = await Promise.all([
    read("app/complete-game.tsx"),
    read("app/game-engine.ts"),
    read("app/game-engine/world-turn-orchestrator.ts"),
  ...
  assert.match(engineRules, /Legacy UI collections are compatibility projections only/);
  assert.match(engineRules, /adaptWorldAdjudication/);
  ...
  assert.match(adjudicatorPrompt, /publicSignals 可以是 0 至 4 条/);
...
test("investigation wording cannot silently create a facility", async () => {
  const [facade, contracts] = await Promise.all([read("app/game-engine.ts"), read("app/game-engine/action-contracts.ts")]);
  assert.match(contracts, /function isExplicitConstruction/);
  assert.match(contracts, /const primaryClause = intent\.split/);
  assert.match(contracts, /explicitKind !== "自由行动" && proposedKind !== explicitKind/);
  assert.match(contracts, /proposedKind === "建设" && !isExplicitConstruction/);
  assert.match(contracts, /facilityId: safeKind === "建设"/);
  assert.doesNotMatch(contracts, /if \(\/建\|修建\|改造\|据点/);
  assert.match(facade, /export \{ interpretIntentWithAi, localContract \} from "\.\/game-engine\/action-contracts\.ts"/);
});
...
test("weekly prose normalizes punctuation and carries consequences into the next council", async () => {
  const [source, resolution, reader, saves, session] = await Promise.all([read("app/game-engine.ts"), read("app/game-engine/week-resolution.ts"), read("app/complete-game.tsx"), read("app/save-system.ts"), read("app/game-session-controller.ts")]);
  const rules = `${source}\n${resolution}`;
  assert.match(rules, /function cleanNarrative/);
  assert.match(rules, /replace\(\/\^若\(\?:继续\(\?:搁置\|放任\)\|不加干预\)/);
  assert.match(rules, /heading: "下一次集会之前"/);
  assert.match(rules, /还剩\$\{pressure\.deadline\}周/);
